...
export async function transcribeAudio(audioPath: string) {
    ...
    try {
        ...
        const response = await axios.post(`${base}/v1/audio/transcriptions`, {
            audio: audioBase64,
        }, {
            headers: {
...
export async function textToSpeech(text: string) {
    try {
        const base = (keys.openaiBase ?? '').replace(/\/+$/, '');
        const response = await axios.post(`${base}/v1/audio/speech`, {
...
export async function describeImage(imagePath: string) {
    ...
    try {
        const base = (keys.openaiBase ?? '').replace(/\/+$/, '');
        const response = await axios.post(`${base}/v1/images/descriptions`, {
            image: imageBase64,
        }, {
            headers: {
                'Authorization': `Bearer ${keys.openai}`,  // Replace YOUR_API_KEY with your actual OpenAI API key
